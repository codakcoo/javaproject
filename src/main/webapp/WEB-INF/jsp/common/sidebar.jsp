...
<style>
    ...
    .sb-user { padding: 10px 14px 8px; border-bottom: 1px solid var(--border-lt); background: #FAFAFA; }
    ...
    .nav-group-title {
        font-size: 11px; font-weight: 700; color: var(--muted); letter-spacing: 0.5px;
        padding: 8px 14px 4px; background: #FAFAFA; border-bottom: 1px solid var(--border-lt);
    }
    .nav-item {
        display: flex; align-items: center; gap: 8px;
        padding: 7px 14px; color: var(--text-sm); font-size: 12px;
    ...
    .nav-toggle svg.arr { width: 12px; height: 12px; fill: var(--muted); transition: transform 0.2s; flex-shrink: 0; }
    ...
    .nav-sub .nav-item { padding-left: 34px; font-size: 12px; color: var(--text-sm); border-left: none; border-bottom: 1px solid var(--border-lt); }
    .nav-sub .nav-item:last-child { border-bottom: none; }
    .nav-sub .nav-item::before { content: ""; width: 4px; height: 4px; background: var(--muted); border-radius: 50%; flex-shrink: 0; }
    ...
    .sb-ver { padding: 8px 14px; font-size: 10px; color: var(--muted); border-top: 1px solid var(--border-lt); background: #FAFAFA; }
...
<nav id="sidebar">
    ...
    <!-- 메인 -->
    <div class="nav-group">
        <a href="${pageContext.request.contextPath}/main.do" class="nav-item" onclick="closeSidebar()">
...
(function () {
    ...
    document.querySelectorAll('#sidebar a.nav-item').forEach(function (link) {
        /* href 끝에 .do 포함 경로와 현재 pathname 비교 */
