...
<style>
    ...
    .info-tip { font-size:11px; color:var(--muted); margin-top:12px; padding:8px 12px; background:#EFF6FF; border-left:3px solid #3B82F6; }
...
<main id="content">
    <div class="page-header">
        <div>
            <div class="page-title">공급업체 관리</div>
            <div class="page-sub">영업/재고 &gt; 거래처 관리 &gt; 공급업체 (발주)</div>
        </div>
        <button class="btn btn-primary" onclick="openInsertModal()">+ 공급업체 등록</button>
    </div>

    <div class="info-tip">
        공급업체는 <strong>발주(입고) 결재서 작성</strong> 시 거래처 드롭다운에 표시됩니다.
        비활성화 처리 시 기존 결재 이력은 보존되며, 신규 결재서 작성에서만 제외됩니다.
    </div>

    <form method="get" action="${pageContext.request.contextPath}/partner/supplier/list.do" style="margin-top:12px;">
        <div class="search-bar">
            ...
            <a href="${pageContext.request.contextPath}/partner/supplier/list.do" class="btn btn-outline">초기화</a>
            ...
            <tbody>
                <c:choose>
                    <c:when test="${empty supplierList}">
                        <tr><td colspan="7" style="text-align:center; padding:30px; color:var(--muted);">등록된 공급업체가 없습니다.</td></tr>
                    </c:when>
                    <c:otherwise>
                        <c:forEach var="s" items="${supplierList}">
                            <tr>
                                <td style="color:var(--muted)">${s.supplierId}</td>
                                <td>
                                    <a href="${pageContext.request.contextPath}/partner/supplier/trades.do?supplierId=${s.supplierId}"
                                       class="partner-link">
                                        <strong>${s.supplierName}</strong>
                                        <span class="trade-hint">거래이력 →</span>
                                    </a>
                                </td>
                                <td>${empty s.contact ? '-' : s.contact}</td>
                                <td>${empty s.phone   ? '-' : s.phone}</td>
                                <td style="max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"
                                    title="${s.address}">${empty s.address ? '-' : s.address}</td>
                                <td>
                                    <span class="badge ${s.isActive eq 1 ? 'badge-active' : 'badge-inactive'}">
                                        ${s.isActive eq 1 ? '활성' : '비활성'}
                                    </span>
                                </td>
                                <td>
                                    <div class="action-btns">
                                        <button class="btn-edit"
                                            onclick="openUpdateModal(${s.supplierId},'${s.supplierName}','${s.contact}','${s.phone}','${s.address}',${s.isActive})">
                                            수정
                                        </button>
                                        <c:if test="${s.isActive eq 1}">
                                            <button class="btn-del" onclick="doDeactivate(${s.supplierId},'${s.supplierName}')">비활성화</button>
                                        </c:if>
                                    </div>
                                </td>
                            </tr>
...
<div class="modal-bg" id="insertModal">
    <div class="modal">
        <div class="modal-title">공급업체 등록</div>
        <form method="post" action="${pageContext.request.contextPath}/partner/supplier/insert.do">
            <div class="form-row">
                <label>업체명 *</label>
                <input type="text" name="supplierName" required placeholder="예) (주)한국부품">
            </div>
            <div class="form-2col">
                <div class="form-row">
                    <label>담당자</label>
                    <input type="text" name="contact" placeholder="예) 이철수">
                </div>
                <div class="form-row">
                    <label>전화번호</label>
            ...
            <div class="form-row">
                <label>주소</label>
                <input type="text" name="address" placeholder="예) 대전광역시 서구 ...">
            </div>
...
<div class="modal-bg" id="updateModal">
    <div class="modal">
        <div class="modal-title">공급업체 수정</div>
        <form method="post" action="${pageContext.request.contextPath}/partner/supplier/update.do">
            <input type="hidden" name="supplierId" id="u_id">
            <div class="form-row">
                <label>업체명 *</label>
                <input type="text" name="supplierName" id="u_name" required>
            </div>
            <div class="form-2col">
                <div class="form-row">
                    <label>담당자</label>
...
<!-- 비활성화 폼 -->
<form id="deactivateForm" method="post" action="${pageContext.request.contextPath}/partner/supplier/deactivate.do">
    <input type="hidden" name="supplierId" id="del_id">
</form>
