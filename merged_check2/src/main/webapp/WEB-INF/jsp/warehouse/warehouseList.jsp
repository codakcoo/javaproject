...
<style>
    ...
    .search-card input { width:200px; } .search-card select { width:140px; }
    ...
    .btn-edit { background:#EFF6FF;color:#2563EB;border:none;padding:4px 10px;border-radius:6px;
                font-size:12px;font-family:inherit;cursor:pointer;font-weight:500;transition:background .15s; }
    .btn-edit:hover { background:#DBEAFE; }
    .btn-del  { background:#FFF1F2;color:#E11D48;border:none;padding:4px 10px;border-radius:6px;
                font-size:12px;font-family:inherit;cursor:pointer;font-weight:500;transition:background .15s; }
...
<main id="content">
    <div class="page-header">
        <div>
            <div class="page-title">창고 관리</div>
            <div class="page-sub">등록된 창고 목록을 조회하고 관리합니다.</div>
        </div>
        <a href="${pageContext.request.contextPath}/warehouse/insertForm.do" class="btn btn-success">+ 창고 등록</a>
    </div>

    <form action="${pageContext.request.contextPath}/warehouse/list.do" method="get">
    <div class="search-card">
        <select name="type">
            <option value="">전체 유형</option>
            <option value="MAIN"    <c:if test="${searchVO.type == 'MAIN'}">selected</c:if>>MAIN (본창고)</option>
            <option value="DEFECT"  <c:if test="${searchVO.type == 'DEFECT'}">selected</c:if>>DEFECT (불량)</option>
            <option value="RETURN"  <c:if test="${searchVO.type == 'RETURN'}">selected</c:if>>RETURN (반품)</option>
            <option value="TRANSIT" <c:if test="${searchVO.type == 'TRANSIT'}">selected</c:if>>TRANSIT (이동중)</option>
        </select>
        ...
        <input type="text" name="keyword" value="${searchVO.keyword}" placeholder="창고명 또는 코드 검색">
        <button type="submit" class="btn btn-primary">검색</button>
        <a href="${pageContext.request.contextPath}/warehouse/list.do" class="btn btn-outline">초기화</a>
    </div>
    </form>
    ...
    <div class="table-card">
        <div class="table-card-head">
            <span>창고 목록</span>
            <span class="total-badge">총 <strong>${empty warehouseList ? '0' : warehouseList.size()}</strong>건</span>
        </div>
        <table>
            <thead>
                <tr>
                    <th>창고코드</th><th>창고명</th><th>유형</th><th>위치</th><th>상태</th><th>관리</th>
                </tr>
            </thead>
            <tbody>
                <c:choose>
                    <c:when test="${empty warehouseList}">
                        <tr class="empty-row"><td colspan="6">등록된 창고가 없습니다.</td></tr>
                    </c:when>
                    <c:otherwise>
                        <c:forEach items="${warehouseList}" var="w">
                        <tr>
                            <td><strong>${w.warehouseCode}</strong></td>
                            <td>${w.warehouseName}</td>
                            <td><span class="type-badge">${w.type}</span></td>
                            <td style="color:var(--muted)">${w.location}</td>
                            <td>
                                <c:choose>
                                    <c:when test="${w.isActive == 1}"><span class="status-badge status-active">활성</span></c:when>
                            ...
                            <td>
                                <div class="action-btns">
                                    <button class="btn-edit"
                                        onclick="location.href='${pageContext.request.contextPath}/warehouse/updateForm.do?warehouseId=${w.warehouseId}'">수정</button>
                                    <form action="${pageContext.request.contextPath}/warehouse/delete.do" method="post" style="display:inline">
                                        <input type="hidden" name="warehouseId" value="${w.warehouseId}">
                                        <button type="submit" class="btn-del"
                                            onclick="return confirm('\'${w.warehouseName}\' 창고를 비활성화하시겠습니까?')">삭제</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
